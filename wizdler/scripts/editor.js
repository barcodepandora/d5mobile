...
var AppEditor = {
	...
	onGoClick: function(e) {
		...
		AppEditor.ajax({
			...
			url: "http://uzupisweb001.azurewebsites.net/Service1.svc?wsdl",
			type: "POST",
			contentType: 'text/xml; charset=utf-8',
			dataType: 'xml',
			data: "<Envelope xmlns=\"http://schemas.xmlsoap.org/soap/envelope/\"><Body><hola xmlns=\"http://tempuri.org/\"/><oficina>1</oficina></hola></Body></Envelope>",
			headers: { "SOAPAction": "\"http://tempuri.org/IService1/hola\"", "Content-Type": "text/xml; charset=\"utf-8\"" },
	...
	loadState: function() {
		...
		document.body.classList.toggle('remember-requests', true);
		alert(JSON.stringify(state.request));
	...
	open: function() {
		/*alert("open " + this.addr);
		var ctx = this.ctxs[this.addr];
		alert( "ctx es " + ctx.wsdl );
		cache = null; // Enable garbage collection
		if (!ctx)
			throw new Error('Invalid address to open in editor: ' + this.addr);

		this.request = Wsdl.generateRequest(ctx);
		this.request.body = "<Envelope xmlns=\"http://schemas.xmlsoap.org/soap/envelope/\"><Body><ObtenerOficinasSistema xmlns=\"http://tempuri.org/\"/>    </Body></Envelope>";
		alert( "request es " + JSON.stringify(this.request) );
		$('#method').val(this.request.method);
		$('#address').val(this.request.url);
		$('#request-headers').val(this.serializeHeaders(this.request.headers));
		this.requestEditor.getSession().setValue(this.request.body);
